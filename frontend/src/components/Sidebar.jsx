...
const Sidebar = () => {
  const { getUsers, users, selectedUser, setSelectedUser, isUsersLoading, userNotifications } = useChatStore();
  const { onlineUsers = [] } = useAuthStore();
      ...
      <div className="overflow-y-auto w-full py-3">
        {filteredUsers.map((user) => (
          ...
          >
            ...
            <div className="flex items-center gap-2">
              {userNotifications[user._id] > 0 && (
                <div className="bg-red-500 text-white rounded-full text-xs px-2 py-1">
